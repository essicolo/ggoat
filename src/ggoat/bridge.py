...
class LetsPlotBridge:
    ...
    def _load_js_pyodide(self) -> bool:
        ...
        try:
            ...
            from js import console, document, eval as js_eval
            ...
            if hasattr(__import__('js'), 'LetsPlot'):
                console.log("✅ ggoat: lets-plot JavaScript library loaded successfully")
    ...
    def _load_js_jupyter(self) -> bool:
        ...
        try:
            ...
            try:
                import lets_plot as lp  # type: ignore
                # If import succeeds, we assume the frontend is already configured
                self._js_loaded = True
                return True
            except ImportError:
                ...
                (function() {
                    if (typeof LetsPlot !== 'undefined') {
                        console.log('✅ ggoat: lets-plot JavaScript library loaded in Jupyter');
                    } else {
                        console.log('❌ ggoat: LetsPlot object not found after loading');
                    }
                })();
    ...
    def _render_pyodide(self, plot_spec: Dict[str, Any]) -> str:
        ...
        from js import LetsPlot, document, console
        ...
        try:
            ...
            container_html = f'<div id="{plot_id}" style="width: 100%; height: 400px;"></div>'
    ...
    def _render_jupyter(self, plot_spec: Dict[str, Any]) -> str:
        ...
        try:
            ...
            try:
                ...
                # Convert our spec to a native lets_plot object
                ggoat_spec = self._convert_to_lets_plot_native(plot_spec)
                
                # Return the lets_plot object directly - it will handle its own rendering
                return ggoat_spec
                
            except ImportError:
                # lets_plot not available, use our JavaScript approach
                pass
            ...
            html_content = f"""
            <div id="{plot_id}" style="width: 100%; height: 400px; border: 1px solid #ccc;"></div>
            <script>
            try {{
                const spec = {plot_json};
                const plotDiv = document.getElementById('{plot_id}');
                if (plotDiv && typeof LetsPlot !== 'undefined') {{
                    ...
                    console.error('❌ Plot container or LetsPlot not found in Jupyter');
                    plotDiv.innerHTML = '<p style="color: red;">Error: LetsPlot not loaded</p>';
                }}
            }} catch (e) {{
                console.error('❌ Error rendering plot in Jupyter:', e);
                document.getElementById('{plot_id}').innerHTML = '<p style="color: red;">Error rendering plot: ' + e.message + '</p>';
            }}
            </script>
            """
        ...
        if facet:
            try:
                ...
                if ftype == "wrap":
                    ...
                    vars_list = vars_raw if isinstance(vars_raw, (list, tuple)) else [vars_raw]
                    if hasattr(lp, "facet_wrap"):
                        try:
                            if len(vars_list) == 1:
                                plot = plot + lp.facet_wrap(vars_list[0], nrow=facet.get("nrow"), ncol=facet.get("ncol"), scales=facet.get("scales"))
                            else:
                                plot = plot + lp.facet_wrap(vars_list, nrow=facet.get("nrow"), ncol=facet.get("ncol"), scales=facet.get("scales"))
                        except TypeError:
                    ...
                    if hasattr(lp, "facet_grid"):
                        try:
                            ...
                            # In lets-plot: x=columns, y=rows (opposite of our rows/cols naming)
                            plot = plot + lp.facet_grid(x=col_var, y=row_var, scales=facet.get("scales"))
                        except Exception:
                            # If facet_grid fails, skip faceting rather than create misleading combined strips
                            pass
            except Exception:
        ...
        if coord:
            try:
                ...
                if ctype == 'cartesian' and hasattr(lp, 'coord_cartesian'):
                    try:
                        plot = plot + lp.coord_cartesian(xlim=coord.get('xlim'), ylim=coord.get('ylim'), expand=coord.get('expand', True))
                    except TypeError:
                        plot = plot + lp.coord_cartesian(coord.get('xlim'), coord.get('ylim'))
                elif ctype == 'flip' and hasattr(lp, 'coord_flip'):
                    try:
                        plot = plot + lp.coord_flip(xlim=coord.get('xlim'), ylim=coord.get('ylim'), expand=coord.get('expand', True))
                    except TypeError:
                        plot = plot + lp.coord_flip(coord.get('xlim'), coord.get('ylim'))
                elif ctype == 'fixed' and hasattr(lp, 'coord_fixed'):
                    try:
                        plot = plot + lp.coord_fixed(ratio=coord.get('ratio', 1), xlim=coord.get('xlim'), ylim=coord.get('ylim'), expand=coord.get('expand', True))
                    except Exception:
                        pass
                elif ctype == 'polar' and hasattr(lp, 'coord_polar'):
                    try:
                        plot = plot + lp.coord_polar(theta=coord.get('theta', 'x'), start=coord.get('start', 0), direction=coord.get('direction', 1))
                    except Exception:
                        pass
                elif ctype == 'map' and hasattr(lp, 'coord_map'):
                    try:
                        plot = plot + lp.coord_map(projection=coord.get('projection', 'mercator'), xlim=coord.get('xlim'), ylim=coord.get('ylim'), expand=coord.get('expand', True))
                    except Exception:
                        pass
            except Exception:
                pass
            ...
            try:
                ...
                if aesthetic == 'color':
                    if scale_type == 'manual':
                        ...
                        if values:
                            plot = plot + lp.scale_color_manual(values=values, 
                                                              name=scale.get('name'),
                        ...
                        plot = plot + lp.scale_color_gradient(low=scale.get('low', '#132B43'),
                    ...
                    if scale_type == 'manual':
                        values = scale.get('values')
                        if values:
                            plot = plot + lp.scale_fill_manual(values=values,
                                                             name=scale.get('name'),
                                                             breaks=scale.get('breaks'),
                                                             labels=scale.get('labels'),
                                                             limits=scale.get('limits'),
                        ...
                        plot = plot + lp.scale_fill_gradient(low=scale.get('low', '#132B43'),
                                                           high=scale.get('high', '#56B1F7'),
                                                           name=scale.get('name'),
                                                           breaks=scale.get('breaks'),
                                                           labels=scale.get('labels'),
        ...
        if theme:
            try:
                ...
                if isinstance(theme, dict) and 'flavor' in theme:
                    ...
                    elif flavor == 'high_contrast_dark' and hasattr(lp, 'flavor_high_contrast_dark'):
                        plot = plot + lp.flavor_high_contrast_dark()
                    elif flavor == 'high_contrast_light' and hasattr(lp, 'flavor_high_contrast_light'):
                        plot = plot + lp.flavor_high_contrast_light()
                    elif flavor == 'solarized_dark' and hasattr(lp, 'flavor_solarized_dark'):
                        plot = plot + lp.flavor_solarized_dark()
                    elif flavor == 'solarized_light' and hasattr(lp, 'flavor_solarized_light'):
                ...
                pass
        # Tooltips, font features, sampling: (pass as metainfo for lets-plot JS or as layer params if supported)
        # Not all are natively supported in lets-plot Python API, but can be passed in JS spec
        # Add to metainfo_list if present
        for meta_key in ["tooltips", "font_metrics", "font_family_info", "sampling"]:
            meta_val = getattr(plot_spec, meta_key, None) if hasattr(plot_spec, meta_key) else plot_spec.get(meta_key)
            if meta_val:
                try:
                    if hasattr(plot, 'add_meta'):  # lets-plot >=4.0
                        ...
                        if hasattr(plot, 'metainfo_list'):
                            plot.metainfo_list.append({"name": meta_key, "value": meta_val})
                except Exception:
                    pass
    ...
    def _render_fallback(self, plot_spec: Dict[str, Any]) -> str:
        ...
        return f"""
        <div id="{plot_id}" style="padding: 20px; border: 2px dashed #ccc; text-align: center;">
            <h3>🐐 ggoat Plot</h3>
            <p><strong>Environment:</strong> Standalone Python (limited functionality)</p>
            <p>For full plot rendering, use in Pyodide or Jupyter environment.</p>
            <details>
...
{spec_json}
    ...
    def _convert_to_lets_plot_spec(self, plot_spec: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if facet:
            ...
            normalized = dict(facet) if isinstance(facet, dict) else {"type": "wrap", "vars": facet}
            ...
            try:
                lets_plot_spec["metainfo_list"].append({"name": "facets", "value": normalized})
            except Exception:
                pass
        ...
        if coord:
            ...
            try:
                lets_plot_spec["metainfo_list"].append({"name": "coord", "value": coord})
            except Exception:
                pass
        ...
        for layer in plot_spec.get("layers", []):
            ...
            kind = layer.get("kind")
            if kind in {"image_matrix", "corr_plot", "qq_plot", "joint_plot", "residual_plot", "waterfall_plot"}:
    ...
    def save_plot(self, plot_spec: Dict[str, Any], filename: str, 
                  width: int = 600, height: int = 400, format: str = 'html') -> bool:
        """
        Save plot to file.
    ...
    def _create_standalone_html(self, plot_spec: Dict[str, Any], 
...
<body>
    ...
    <div class="footer">
        <p>Rendered using <a href="https://github.com/JetBrains/lets-plot" target="_blank">lets-plot</a> JavaScript library</p>
    </div>
    ...
    <script>
        try {{
            ...
            if (typeof LetsPlot !== 'undefined') {{
                ...
                plotDiv.innerHTML = '<p style="color: red;">Error: LetsPlot library not loaded</p>';
                console.error('❌ LetsPlot library not found');
            }}
        }} catch (e) {{
            console.error('❌ Error rendering plot:', e);
            document.getElementById('plot').innerHTML = '<p style="color: red;">Error rendering plot: ' + e.message + '</p>';
        }}
    </script>
</body>
</html>
